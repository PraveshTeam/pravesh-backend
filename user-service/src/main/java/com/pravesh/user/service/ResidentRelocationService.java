...
public class ResidentRelocationService {
    ...
    public RelocationRequestResponse createRequest(CreateRelocationRequest req, Long residentUserId,
        ...
        requestRepository.findByResidentUserIdAndStatus(residentUserId, RequestStatus.PENDING)
                .ifPresent(r -> {
        ...
        ResidentRelocationRequest request = ResidentRelocationRequest.builder()
                .residentUserId(residentUserId)
                .oldFlatId(oldFlat.getId())
                .oldSocietyId(oldFlat.getSocietyId())
                .targetSocietyId(req.targetSocietyId())
    ...
    public List<RelocationRequestResponse> getRequestsForSociety(Long societyId, RequestStatus status) {
        return requestRepository.findByTargetSocietyIdAndStatusOrderByCreatedAtDesc(societyId, status)
    ...
    public RelocationRequestResponse approve(Long requestId, Long reviewerId, boolean force) {
        ...
        Flat newFlat = flatRepository
                ...
                .orElseGet(() -> flatRepository.save(Flat.builder()
                        .societyId(request.getTargetSocietyId())
        ...
        if (occupiedByOther && !force) {
            String occupantName = userRepository.findById(newFlat.getResidentId())
                    .map(User::getName).orElse("Unknown resident");
        ...
        if (occupiedByOther) {
            ...
            if (displaced != null) {
                ...
                String displacedName = displacedUser != null ? displacedUser.getName() : "resident #" + displaced.getUserId();
        ...
        historyRepository.save(ResidentRelocationHistory.builder()
                .residentUserId(request.getResidentUserId())
                .oldFlatId(request.getOldFlatId())
                .oldSocietyId(request.getOldSocietyId())
                .newFlatId(newFlat.getId())
                .newSocietyId(request.getTargetSocietyId())
                .approvedBy(reviewerId)
                .build());

        try {
            String oldFlatNumber = flatRepository.findById(request.getOldFlatId())
                    .map(Flat::getFlatNumber).orElse("—");
            String oldSocietyName = societyRepository.findById(request.getOldSocietyId())
                    .map(Society::getName).orElse("—");
            String newSocietyName = societyRepository.findById(request.getTargetSocietyId())
                    .map(Society::getName).orElse("—");

            notificationFeignClient.notifyRelocationApproved(new RelocationApprovedRequest(
                    request.getResidentUserId(),
                    newFlat.getFlatNumber(), newFlat.getTower(), newSocietyName,
                    oldFlatNumber, oldSocietyName));
        } catch (Exception e) {
            log.warn("Failed to notify resident {} of relocation approval: {}",
                    request.getResidentUserId(), e.getMessage());
        }
    ...
    private RelocationRequestResponse toResponse(ResidentRelocationRequest r) {
        String residentName = userRepository.findById(r.getResidentUserId())
                .map(User::getName).orElse("Unknown");
        String oldFlatNumber = flatRepository.findById(r.getOldFlatId())
                .map(Flat::getFlatNumber).orElse("—");
        String oldSocietyName = societyRepository.findById(r.getOldSocietyId())
                .map(Society::getName).orElse("—");
        String targetSocietyName = societyRepository.findById(r.getTargetSocietyId())
                .map(Society::getName).orElse("—");

        return new RelocationRequestResponse(
                r.getId(), residentName, oldFlatNumber, oldSocietyName,
                r.getClaimedFlatNumber(), targetSocietyName, r.getDocumentType(),
                r.getStatus(), r.getAdminNotes(), r.getCreatedAt());
    }
}